...
namespace ObservableBinding.WPF
{
  public static class OEmit
  {
    ...
    //{
    //  var propName = "HasFocus";
    //  var newObserver = e.NewValue as IObserver<bool>;
    //  IDisposable newSub = null;

    //  //if (d is UIElement el && newObserver != null)
    //  //{
    //  //  var gotFocusStream = Observable.FromEventPattern<RoutedEventArgs>(el, nameof(el.GotFocus))
    //  //                        .Select(patt => true);
    //  //  var lostFocusStream = Observable.FromEventPattern<RoutedEventArgs>(el, nameof(el.LostFocus))
    //  //                        .Select(patt => false);

    //  //  newSub = Observable.Merge(gotFocusStream, lostFocusStream)
    //  //    .StartWith(el.IsFocused)
    //  //    .Subscribe(newObserver);
    //  //}

    //  if (d is UIElement el && newObserver != null)
    //  {
    //    newSub = d.Observe(UIElement.IsFocusedProperty)
    //              .Select(ea => (bool)d.GetValue(UIElement.IsFocusedProperty))
    //              .Subscribe(newObserver);
    //  }
